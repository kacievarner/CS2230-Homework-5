...
public abstract class AbstractBinaryTree<E> extends AbstractTree<E>
  ...
  public E reduceAtDepthRecursive(int depth, BiFunction<E, E, E> f, E initialValue) {
    ...
    if (!this.isEmpty()) reduceHelper(depth, this.root(), null, nodes);
      for(Position<E> i: nodes){
        initialValue = f.apply(initialValue, i.getElement());
      }
      return initialValue;
    }

  private void reduceHelper(int placeInt, Position<E> x, E soFar, List<Position<E>> linkedList) {
    if (x == null) {
      return;
    }
    if (placeInt == 0) {
      ...
      reduceHelper(placeInt - 1, left(x), soFar, linkedList);
      reduceHelper(placeInt - 1, right(x), soFar, linkedList);
    }
  }
  /* Returns the result of combining all elements at the given depth
  using the given BiFunction and initialValue
  ...
  public E reduceAtDepthIterative(int depth, BiFunction<E, E, E> f, E initialValue) {
    ...
    Queue<Position<E>> treeNodes = new LinkedQueue<>(); //Queue to track nodes
    treeNodes.enqueue(this.root());
    while (depth > 0) {
      int count = treeNodes.size();
      depth -= 1;
      while (count > 0) {
        Position<E> currentNode = treeNodes.dequeue();
        if (left(currentNode) != null) {
          treeNodes.enqueue(left(currentNode));
        }
        if (right(currentNode) != null) {
          treeNodes.enqueue(right(currentNode));
        }
        count -= 1;
      }
    }
    while (treeNodes.size() > 0){
      initialValue = f.apply(initialValue, treeNodes.dequeue().getElement());
    }
    return initialValue;
  }
  ...
  public List<E> suchThatIterative(BiFunction<BinaryTree<E>, Position<E>, Boolean> p) {
    Stack<Position<E>> treeNodes = new Stack<>();
    List<E> finalNodes = new LinkedList<>();
    if (!this.isEmpty()) treeNodes.push(this.root());
    while (!treeNodes.isEmpty()) {
      Position<E> posNode = treeNodes.pop();
      if (left(posNode) != null) {
        treeNodes.push(left(posNode));
      }
      if (right(posNode) != null) {
        treeNodes.push(right(posNode));
      }
      if (p.apply(this, posNode)) {
        finalNodes.add(posNode.getElement());
      }
    }
    return finalNodes;
  }
